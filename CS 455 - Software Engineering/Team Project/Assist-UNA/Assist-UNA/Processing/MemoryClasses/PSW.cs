 ...
 * 04/15/2014   JMB     Added new SetBytes method.
...
namespace Assist_UNA
{
    class PSW
    {
         ...
         * Author(s):   Michael Beaver  
         *                         
         * Input:       The index is an unsigned integer, and value is an unsigned integer.   
         * Return:      The result is boolean.     
         * Description: This method will attempt to set a contiguous sequence of bytes starting at
         *              "index" to "value." If an error occurs, the returned result will be false. 
         *              Otherwise, the returned result will be true.
         *              
         *****************************************************************************************/
        public bool SetBytes(uint index, uint value)
        {
            bool result = false;
            int i = 0;
            int val;
            ...
            else
            {
                ...
                try
                {
                    hexValue = value.ToString("X").PadLeft(6, '0');

                    while (i < (LOC_SIZE * 2))
                    {
                        val = Convert.ToSByte(hexValue[i].ToString() + 
                            hexValue[i + 1].ToString(), 16);
                        
                        result = locContents[index + offset].SetValue(val);

                        if (index + offset == LOC_SIZE || result == false)
                            break;
