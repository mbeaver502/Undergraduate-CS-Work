...
9 March 2015
Program Description: 
	This program implements the client side of the Knock-knock protocol.
	This client prompts the server side for a knock-knock joke. The server
	and client, in turn, send and receive messages corresponding to the
	protocol until an error occurs or until the joke is completed. The
	client executes for only one joke at a time.
Resources consulted:
	Various man pages	
*/
...
/*
Function: main
Purpose: This function provides the main loop for the sending and receiving
	of the knock-knock joke messages.
Parameters: N/A
...
int main(int argc, char * argv[]) {
	...
	while (jokeState != STATE_PUNCH && jokeState != STATE_ERROR) {
		...
		if (recv(s, buffer, CODE_LEN + MAX_MSG, 0) < 1) {
			...
			// A bit redundant, but just in case 
...
Purpose: This function is used to generically send codes and text (as 
    part of the protocol's message format) to the server.
...
Purpose: This function will appropriately alter the jokeState of the client 
    based on the given code.
Parameters: code is the C-string specifying the protocol code, jokeState is
    the state of the client that will be altered
Return: N/A (void)
*/
void setJokeState(char code[], int & jokeState) {
...
Purpose: This function will output the next part of the joke to the client
    and send that part to the server. Also, the joke state is updated.
Parameters: new_s is the open socket, jokeState is the joke state of the
    client to be updated, msg is the message struct containing the message
Return: N/A (void)
*/
void jokeHandler(int new_s, int & jokeState, struct message * msg) {
	...
	switch(jokeState) {
		...
		// Send the "300 string" who message
		case STATE_HOOK: {
			jokeState = STATE_REPLY;
			char temp[MAX_MSG + 4];
		...
		// The joke is complete after the punchline
		case STATE_PUNCH: {
			jokeState = STATE_PUNCH;
			break;
		}
...
Purpose: This function will verify the correctness of a message from the 
    server and store the two parts of the message (code and text) in a 
    message struct object.
Parameters: msg is a pointer to the message struct to hold the server's message,
    buffer is the server's message (potentially MAX_MSG + CODE_LEN bytes)
