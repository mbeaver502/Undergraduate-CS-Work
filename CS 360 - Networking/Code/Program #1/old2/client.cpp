...
*/
	...
	jokeState = 0;
	sendMessage(s, CODES[STATE_INIT], "Tell me a joke");
	printf("%s Tell me a joke\n", CODES[STATE_INIT]);
	...
	while (jokeState != STATE_PUNCH && jokeState != STATE_ERROR) {
		...
		if (recv(s, buffer, CODE_LEN + MAX_MSG, 0) < 1) {
			...
			sendMessage(s, CODES[STATE_ERROR], "Fuck you");
...
void jokeHandler(int new_s, int & jokeState, struct message * msg) {
	...
	switch(jokeState) {
		...
		case STATE_KNOCK: {
			jokeState = STATE_WHO;
			sendMessage(new_s, CODES[jokeState], "Who is there");
			printf("%s Who is there\n", CODES[jokeState]);;
			break;
		}
		...
		case STATE_HOOK: {
			jokeState = STATE_REPLY;
			char temp[MAX_MSG + 4];
			strlcpy(temp, msg->text, strlen(msg->text));
			strncat(temp, " who", 4);
